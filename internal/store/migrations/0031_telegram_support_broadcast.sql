-- Telegram support relay, mass broadcasts and user notifications (issue #29).
...
-- ---------------------------------------------------------------------------
-- Notifications the panel sends to the USER
-- ---------------------------------------------------------------------------
...
-- both the worst moment to find out and the point at which they write to support
-- instead of renewing.
ALTER TABLE settings ADD COLUMN tg_user_events INTEGER NOT NULL DEFAULT -1;

-- How many days before expiry the warning goes out. One knob rather than a set of
-- thresholds: two reminders for one expiry read as nagging, and the operator can pick
-- the horizon that matches their renewal flow.
...
-- on every poll. Holding the expiry itself (not a timestamp) re-arms it for free:
-- renewing moves expire_at, the stored value stops matching, and the next cycle is
-- eligible again — with no way to leave it stuck armed or stuck spent.
...
-- expiry there is no changing value to key off. 0 = not yet warned; cleared again
-- whenever usage drops back under the threshold, which is exactly what a quota reset
-- or a move to a bigger plan does.
