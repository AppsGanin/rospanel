...
// AddUserEvents appends several journal rows in one transaction — what a bulk
// action needs. One row per user through AddUserEvent means one transaction per
// user on a store with a single connection: five hundred users selected in the
// panel became five hundred round trips, holding that connection while every
// other request waited.
func (s *Store) AddUserEvents(evs []model.UserEvent) error {
	...
	return s.withTx(func(tx *sql.Tx) error {
		...
		for _, ev := range evs {
			...
			if _, err := stmt.Exec(ev.UserID, ev.UserName, ev.Action, ev.ActorKind, ev.ActorName, raw, at); err != nil {
				return err
			}
		}
		return nil
