...
- **Админ-бот**: список пользователей, добавление, удаление, вкл/выкл, сброс
  трафика, **QR-подписка** по юзеру; **бэкапы по расписанию** (cron) и по команде.
...
- **Пользовательский бот** (отдельный токен): открытый бот для клиентов —
  **саморегистрация** (получают подписку сами), личное меню с подпиской, QR и
  статистикой; привязка существующего аккаунта по **deep-link**. Саморегистрацию
  можно включать/выключать. Ссылка на бота прокидывается в клиенты как support-url
  (кнопка поддержки в Happ)
