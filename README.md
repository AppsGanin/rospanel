...
перезагрузкой Xray. Дашборд — CPU / RAM / swap / диск / сеть / VPN-трафик в реальном времени.
...
уведомления о событиях (регистрации, истечения, сбой Xray, оплаты, сертификат). **Клиентский
бот**: саморегистрация, личное меню с подпиской и статистикой, покупка тарифа. **REST API** с
именованными ключами (`Authorization: Bearer`), **OpenAPI из кода** и Swagger UI; **вебхуки**
шлют подписанные HMAC-SHA256 события с ретраями. Подробнее — [docs/api.md](docs/api.md).
