...
`/dev/net/tun` и `CAP_NET_ADMIN` (у установленного сервиса они есть) и `nftables` для NAT.
...
сопоставлялась с аккаунтом. Поиск и фильтры не тормозят на сотнях
...
журнал панели ложится, кто что завершил.
